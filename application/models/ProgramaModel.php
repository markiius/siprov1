...
class ProgramaModel extends CI_Model {
    ...
    //marco
    public function getFirma($id_usuario, $id_mes, $id_programa){
        $this->db->select("*");
        $this->db->from("firmas_reporte");
        $this->db->where("idusuario", $id_usuario);
        $this->db->where("mes", $id_mes);
    ...
    public function setFirma($idusuario,$mes,$idprograma,$firma){
        $datos = array(
            "idusuario" => $idusuario,
            "mes" => $mes,
        ...
        $this->db->insert('firmas_reporte', $datos);
		if ($this->db->affected_rows() > 0) {
